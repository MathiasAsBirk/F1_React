...
export default function News() {
  ...
  const saveNews = (next) => {
    ...
    try {
      ...
      console.warn("Failed to save news:", e);
    }
  };

  const openModal = (item) => {
    setActiveNews(item);
    if (editMode) setEditDraft({ ...item }); // clone for editing
  };

  const handleSave = () => {
    const next = news.map(n => n.id === editDraft.id ? editDraft : n);
    saveNews(next);
    setActiveNews(null);
    setEditDraft(null);
  };
  ...
  return (
    <section className={styles.newsWrapper}>
      <h2 className={styles.sectionTitle}>
        F1 News
        <span style={{ float: "right", fontSize: "1rem", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {editMode && (
            <>
              <button onClick={handleAdd} className={styles.closeButton} style={{ padding: "6px 10px" }}>
                + Add
              </button>
              <button onClick={handleExport} className={styles.closeButton} style={{ padding: "6px 10px" }}>
                Export JSON
              </button>
              <button onClick={handleImportClick} className={styles.closeButton} style={{ padding: "6px 10px" }}>
                Import JSON
              </button>
              <button onClick={handleReset} className={styles.closeButton} style={{ padding: "6px 10px", background: "#6b7280" }}>
                Reset
              </button>
              <input
                type="file"
            ...
            {!editMode ? (
              ...
              <>
                <input
                  ...
                  style={{ width: "100%", marginBottom: 8, fontSize: "1.2rem", padding: 6, background: "#111", border: "1px solid #444", color: "#fff", borderRadius: 6 }}
                  placeholder="Title"
                />
                <input
                  value={editDraft.category}
                  onChange={(e) => handleEditChange("category", e.target.value)}
                  style={{ width: "100%", marginBottom: 8, fontSize: "0.95rem", padding: 6, background: "#111", border: "1px solid #444", color: "#fff", borderRadius: 6 }}
                ...
                <textarea
                  ...
                  rows={2}
                  style={{ width: "100%", marginBottom: 8, fontSize: "0.95rem", padding: 8, background: "#111", border: "1px solid #444", color: "#fff", borderRadius: 6 }}
                  placeholder="Summary"
                />
                <textarea
                  ...
                  rows={10}
                  style={{ width: "100%", fontSize: "1rem", padding: 10, background: "#111", border: "1px solid #444", color: "#fff", borderRadius: 6 }}
